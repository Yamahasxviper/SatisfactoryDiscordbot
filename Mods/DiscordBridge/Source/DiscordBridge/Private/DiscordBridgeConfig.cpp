...
namespace
{
	...
	/**
	 * Builds the full config file text with detailed comments and the current
	 * values from Config.  Used both when creating the file from scratch and
	 * when restoring comments that were stripped by the Alpakit/pak pipeline.
	 */
	FString BuildDetailedConfigString(const FDiscordBridgeConfig& Config)
	{
		const FString bIgnoreBots      = Config.bIgnoreBotMessages        ? TEXT("True") : TEXT("False");
		const FString bShowPresence    = Config.bShowPlayerCountInPresence ? TEXT("True") : TEXT("False");
		const FString IntervalStr      = FString::SanitizeFloat(Config.PlayerCountUpdateIntervalSeconds);
		const FString ActivityTypeStr  = FString::FromInt(Config.PlayerCountActivityType);
		...
		return FString()
			...
			+ TEXT(";  3. Invite the bot to your server with \"Send Messages\" and\n")
			+ TEXT(";     \"Read Message History\" permissions on the target channel.\n")
			+ TEXT(";  4. Enable Developer Mode in Discord (Settings -> Advanced -> Developer Mode),\n")
			+ TEXT(";     right-click the target text channel, and choose \"Copy Channel ID\".\n")
			+ TEXT(";     Paste that value into ChannelId below.\n")
			...
			+ TEXT("; Used as the %ServerName% placeholder in GameToDiscordFormat and\n")
			+ TEXT("; PlayerCountPresenceFormat below.\n")
			...
			+ TEXT("; |   %PlayerName%  - in-game name of the player who sent the message           |\n")
			+ TEXT("; |   %Message%     - the raw chat message text                                 |\n")
			+ TEXT("; |                                                                             |\n")
			+ TEXT("; | Examples:                                                                   |\n")
			...
			+ TEXT("; | Format used as the in-game chat message body when a Discord message is      |\n")
			+ TEXT("; | relayed into Satisfactory.                                                  |\n")
			+ TEXT("; | Available placeholders:                                                     |\n")
			+ TEXT("; |   %Username%    - Discord display name of the sender                        |\n")
			+ TEXT("; |   %PlayerName%  - alias for %Username%                                      |\n")
			+ TEXT("; |   %Message%     - the Discord message text                                  |\n")
			+ TEXT("; |                                                                             |\n")
			+ TEXT("; | Examples:                                                                   |\n")
			+ TEXT("; |   DiscordToGameFormat=%Message%                  (just the message)         |\n")
			+ TEXT("; |   DiscordToGameFormat=%Username%: %Message%      (name + message)           |\n")
			+ TEXT("; |   DiscordToGameFormat=[Discord] %Message%        (tagged message)           |\n")
			+ TEXT("; +-----------------------------------------------------------------------------+\n")
			+ TEXT("DiscordToGameFormat=") + Config.DiscordToGameFormat + TEXT("\n")
			+ TEXT("\n")
			+ TEXT("; +-- DISCORD -> GAME (sender name column) -------------------------------------+\n")
			+ TEXT("; | Format used for the sender name shown in the Satisfactory chat UI when a    |\n")
			+ TEXT("; | Discord message arrives.                                                    |\n")
			+ TEXT("; | Available placeholders:                                                     |\n")
			+ TEXT("; |   %Username%    - Discord display name of the sender                        |\n")
			+ TEXT("; |   %PlayerName%  - alias for %Username%                                      |\n")
			+ TEXT("; |                                                                             |\n")
			...
			+ TEXT("; +-----------------------------------------------------------------------------+\n")
			+ TEXT("DiscordSenderFormat=") + Config.DiscordSenderFormat + TEXT("\n")
			+ TEXT("\n")
			+ TEXT("; -- WHITELIST ----------------------------------------------------------------\n")
			...
			+ TEXT("; Discord channel.  Set to an empty string to disable Discord-based management.\n")
			+ TEXT(";\n")
			...
			+ TEXT(";\n")
			+ TEXT("; When set:\n")
			+ TEXT(";   * Messages in WhitelistChannelId are relayed to game only when the sender\n")
			+ TEXT(";     holds this role.\n")
			+ TEXT(";   * !whitelist role add/remove commands assign or revoke this role via the\n")
			+ TEXT(";     Discord REST API (bot must have Manage Roles permission on your server).\n")
			+ TEXT(";\n")
			+ TEXT("; How to get the role ID:\n")
			+ TEXT(";   Discord Settings -> Advanced -> Developer Mode (enable it), then right-click\n")
			+ TEXT(";   the role in Server Settings -> Roles and choose \"Copy Role ID\".\n")
			...
			+ TEXT(";\n")
			+ TEXT("; When set:\n")
			+ TEXT(";   * In-game messages from players who are on the server whitelist are ALSO\n")
			+ TEXT(";     posted to this channel (in addition to the main ChannelId above).\n")
			+ TEXT(";   * Discord messages sent to this channel are relayed to the game only when\n")
			+ TEXT(";     the sender holds WhitelistRoleId (if WhitelistRoleId is configured).\n")
			+ TEXT(";\n")
			+ TEXT("; Get the channel ID the same way as ChannelId above (right-click the channel\n")
			+ TEXT("; in Discord with Developer Mode enabled -> Copy Channel ID).\n")
			+ TEXT("WhitelistChannelId=") + Config.WhitelistChannelId + TEXT("\n")
			+ TEXT("\n")
			+ TEXT("; Message posted to the main Discord channel when a non-whitelisted player is\n")
			+ TEXT("; kicked.  Leave empty to disable this notification.\n")
			+ TEXT(";\n")
			+ TEXT("; Placeholder:\n")
			+ TEXT(";   %PlayerName%  - in-game name of the player who was kicked.\n")
			+ TEXT(";\n")
			+ TEXT("; Example:\n")
			...
			+ TEXT("bIgnoreBotMessages=") + bIgnoreBots + TEXT("\n")
			+ TEXT("\n")
			...
			+ TEXT("bShowPlayerCountInPresence=") + bShowPresence + TEXT("\n")
			+ TEXT("\n")
			...
			+ TEXT("PlayerCountUpdateIntervalSeconds=") + IntervalStr + TEXT("\n")
			+ TEXT("\n")
			...
			+ TEXT("PlayerCountActivityType=") + ActivityTypeStr + TEXT("\n");
	}
} // anonymous namespace
...
FDiscordBridgeConfig FDiscordBridgeConfig::LoadOrCreate()
{
	...
	if (PlatformFile.FileExists(*ModFilePath))
	{
		...
		// Restore detailed comments if the file was stripped of them (e.g. by
		// Alpakit's pak coalescing pipeline).  We detect stripping by checking
		// whether the file contains any semicolon comment characters.
		FString RawContent;
		if (FFileHelper::LoadFileToString(RawContent, *ModFilePath) &&
		    !RawContent.Contains(TEXT(";")))
		{
			const FString RestoredContent = BuildDetailedConfigString(Config);
			PlatformFile.CreateDirectoryTree(*FPaths::GetPath(ModFilePath));
			if (FFileHelper::SaveStringToFile(RestoredContent, *ModFilePath))
			{
				UE_LOG(LogTemp, Log,
				       TEXT("DiscordBridge: Restored detailed comments to '%s' "
				            "(comments were stripped during packaging)."), *ModFilePath);
			}
			else
			{
				UE_LOG(LogTemp, Warning,
				       TEXT("DiscordBridge: Could not restore comments to '%s'."),
				       *ModFilePath);
			}
		}
	}
	else
	{
		...
		const FString DefaultContent = BuildDetailedConfigString(Config);
