...
FDiscordBridgeConfig FDiscordBridgeConfig::LoadOrCreate()
{
	...
	if (!bLoadedFromMod)
	{
		...
		const FString DefaultContent =
			...
			TEXT("; These settings control commands that can be typed in the Satisfactory\n")
			TEXT("; in-game chat to manage the whitelist and ban list without using Discord.\n")
			TEXT(";\n")
			TEXT("; WARNING: In-game commands have no player-specific authorization check –\n")
			TEXT("; any player currently on the server can type them.  They are disabled by\n")
			TEXT("; default (prefix left empty) to prevent players from running\n")
			TEXT("; '!whitelist off' or '!whitelist add <name>' to bypass enforcement.\n")
			TEXT("; Only enable these if you fully trust everyone already on the server,\n")
			TEXT("; or if you manage access through other means (e.g. a private server).\n")
			TEXT(";\n")
			TEXT("; Prefix that triggers whitelist commands when typed in the in-game chat.\n")
			TEXT("; Leave empty (default) to disable. Set to !whitelist to enable.\n")
			...
			TEXT(";   !whitelist list          - list all whitelisted players\n")
			TEXT(";   !whitelist status        - show current enabled/disabled state\n")
			TEXT("InGameWhitelistCommandPrefix=\n")
			TEXT(";\n")
			TEXT("; Prefix that triggers ban commands when typed in the in-game chat.\n")
			TEXT("; Leave empty (default) to disable. Set to !ban to enable.\n")
			TEXT("; Available commands (type in Satisfactory chat):\n")
			TEXT(";   !ban add <name>    - ban a player by in-game name\n")
			TEXT(";   !ban remove <name> - unban a player by in-game name\n")
			TEXT(";   !ban list          - list all banned players\n")
