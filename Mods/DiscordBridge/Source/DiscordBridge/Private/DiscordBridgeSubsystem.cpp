...
void UDiscordBridgeSubsystem::SendStatusMessageToDiscord(const FString& Message)
{
	...
	PostTextToChannel(Config.ChannelId, Message);
}

void UDiscordBridgeSubsystem::PostTextToChannel(const FString& TargetChannelId,
                                                 const FString& Message)
{
	...
	Request->OnProcessRequestComplete().BindWeakLambda(
		...
		[Message](FHttpRequestPtr /*Req*/, FHttpResponsePtr Resp, bool bConnected)
		{
			...
			{
				UE_LOG(LogTemp, Warning,
				       TEXT("DiscordBridge: HTTP request failed (message truncated to 80 chars): '%.80s'."),
				       *Message);
				return;
			}
                                                        ...
                                                        const FString& Message)
{
	...
	PostTextToChannel(Config.ChannelId, FormattedContent);
}
