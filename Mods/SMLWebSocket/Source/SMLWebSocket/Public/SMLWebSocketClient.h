 ...
 *   2. Bind your callbacks to OnConnected, OnMessage, OnClosed, OnError.
 *   3. Call Connect() with your ws:// or wss:// URL.
 *   4. Use SendText() / SendBinary() to exchange messages.
...
public:
	...
	/** Called on the game thread when the connection is closed (either gracefully or by the remote). */
	UPROPERTY(BlueprintAssignable, Category="SML|WebSocket")
	 ...
	 *
	 * Sends a WebSocket Close frame (RFC 6455 §5.5.1) and waits for the
	 * server's acknowledgement before tearing down the TCP connection.
